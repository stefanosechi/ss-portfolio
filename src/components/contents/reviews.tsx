        ...
        <div className="w-full relative pb-20 py-10 z-40">
            <AnimationContainer animation="slide-up" delay={0.1} className="">
                <div className="w-full">
                    <h2 className="text-2xl lg:text-3xl font-medium text-left w-full">Reviews</h2>
                </div>
            </AnimationContainer>
                ...
                <Carousel className="w-full max-w-full">
                    <CarouselContent>
                        {reviews.map((review: ReviewProps, index: number) => (
                            <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/2 relative">
    ...
    return (
        <Card className="bg-neutral-900/8 border-none p-0 transition-all duration-300 select-none rounded-xl lg:rounded-2xl">
            <CardContent className="flex flex-col h-[22rem] lg:h-80 aspect-video p-4 md:p-6">
                ...
                <p className="text-neutral-300 flex-grow mt-4 text-sm text-justify md:text-lg lg:text-xl leading-relaxed">"{review.text}"</p>
                <div className="mt-auto">
                    <h4 className="text-lg text-foreground font-semibold">{review.name}</h4>
